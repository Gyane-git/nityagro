        ...
        <div className="flex border-b border-gray-200 mb-6">
          {[
            ...
            <button
              key={key}
              onClick={() => setTab(key)}
              className="flex-1 pb-3 text-sm font-semibold relative transition-colors"
              style={{ color: tab === key ? "#266A3F" : "#9CA3AF" }}
            >
              {label}
              {tab === key && (
                <div
                  className="absolute bottom-0 left-0 right-0"
                  style={{ height: "2.5px", background: "#266A3F", borderRadius: "2px 2px 0 0" }}
                />
              )}
            </button>
          ))}
