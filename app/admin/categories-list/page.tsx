...
import { apiDeleteRequest, apiGetRequest, apiPostRequest, apiPutRequest, apiUploadRequest } from "@/apihelper/apiHelper";
...
export default function CategoriesListPage() {
  ...
  const filteredCategories = useMemo(() => {
    ...
    return categories.filter((category) => category.categoryName.toLowerCase().includes(q) || (category.slug || "").toLowerCase().includes(q) || (category.categoryDescription || "").toLowerCase().includes(q));
  }, [categories, search]);
  ...
  const handleOmsSync = async () => {
    ...
    try {
      ...
      const categories: OmsCategory[] = Array.isArray(omsJson?.data) ? omsJson.data : [];
      const uniqueCategoryNames = Array.from(new Set(categories.map((item) => (item.GroupName || "").trim()).filter(Boolean)));
      const products: OmsProduct[] = Array.isArray(omsJson?.data) ? omsJson.data : [];
      const subGroup: OmsSubGroupName[] = Array.isArray(omsJson?.data) ? omsJson.data : [];
      ...
      const product = Object.values(
        products.reduce((acc, item) => {
          ...
          acc[key] = {
            ...
            pImage: null,
            productStatus: false,
            actualPrice: readOmsNumber(item.TradeRate, (item as Record<string, unknown>).tradeRate, (item as Record<string, unknown>).trade_rate, item.BuyRate),
            sellingPrice: readOmsNumber(item.MRP, (item as Record<string, unknown>).Mrp, (item as Record<string, unknown>).mrp, item.SalesRate, (item as Record<string, unknown>).salesRate),
            deliveryTargetDays: null,
          };
          return acc;
      ...
      const productsubGroup = Object.values(
        subGroup.reduce((acc, item) => {
          ...
          acc[key] = {
            ...
            MRP: readOmsNumber(item.MRP, (item as Record<string, unknown>).Mrp, (item as Record<string, unknown>).mrp, (item as Record<string, unknown>).SalesRate, (item as Record<string, unknown>).salesRate),
          };
          return acc;
      ...
      const subCategorySync = await apiPostRequest("/subcategories", requestDataSubGroup);
      if (!categorySync.success || !productSync.success || !subCategorySync.success) {
        toast.error(categorySync.message || productSync.message || subCategorySync.message || "OMS sync failed", { id: syncToastId });
        return;
      }

      toast.success(`OMS sync completed: ${uniqueCategoryNames.length} categories, ${product.length} products, ${productsubGroup.length} variants checked.`, { id: syncToastId });
  ...
  const confirmOmsSync = () => {
    openConfirm({
      ...
      message: "This will refresh categories, products and variants from OMS. New synced items remain inactive until you review and activate them.",
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 mb-4">
          <div className="flex gap-4 items-center">
            <div className="relative text-gray-900">
              ...
              <input type="text" placeholder="Search category..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div className="flex-1 text-center text-2xl text-gray-800 font-semibold">Categories</div>
            <button onClick={confirmOmsSync} disabled={syncingOms} className={`rounded-md px-4 py-2 text-sm font-semibold text-white ${syncingOms ? "bg-gray-400 cursor-not-allowed" : "bg-[#1f6a45] hover:bg-[#155235]"}`}>
              ...
              {filteredCategories.length === 0 ? (
                ...
                filteredCategories.map((category) => (
                  <tr key={category.categoryId} className="hover:bg-green-100 text-sm text-gray-900">
                    ...
                    <td className="p-1 text-center">
                      <span className={`inline-flex rounded-full px-2 py-1 text-xs font-semibold ${category.categoryStatus ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"}`}>{category.categoryStatus ? "Active" : "Inactive"}</span>
                    </td>
                    <td className="px-4 py-2 text-center space-x-2">
                      ...
                      <button onClick={() => handleEditOpen(category)} className="p-1 text-green-600 hover:bg-green-50 rounded-lg transition">
                        <Edit2 size={16} />
                      </button>
                      <button onClick={() => handleDelete(category.categoryId)} className="p-1 text-red-600 hover:bg-red-50 rounded-lg transition">
                        <Trash2 size={16} />
                      </button>
                    </td>
                  </tr>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
          <div className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl border border-gray-200 overflow-hidden">
            <div className="bg-gradient-to-r from-[#0072bc] to-blue-600 px-6 py-5 text-white flex justify-between items-center">
              ...
              <button onClick={() => setInfoDialog(null)} className="w-9 h-9 rounded-full bg-white/20 hover:bg-white/30 transition flex items-center justify-center text-lg">
                ✕
              </button>
            </div>
            ...
            <div className="p-6 space-y-5">
              <div>
                ...
                <div className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-gray-800 font-medium">{infoDialog.categoryName || "-"}</div>
              </div>

              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Slug</label>
                <div className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-gray-800">{infoDialog.slug || "-"}</div>
              </div>

              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Description</label>
                <div className="min-h-[120px] w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-gray-800 whitespace-pre-wrap leading-relaxed">{infoDialog.categoryDescription || "No description available"}</div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="rounded-xl bg-green-50 border border-green-100 p-4">
                  ...
                  <p className={`font-semibold ${infoDialog.categoryStatus ? "text-green-600" : "text-gray-600"}`}>{infoDialog.categoryStatus ? "Active" : "Inactive"}</p>
                </div>
              </div>

              <div className="pt-5 border-t flex justify-end">
                <button onClick={() => setInfoDialog(null)} className="px-6 py-2.5 rounded-xl bg-[#0072bc] text-white font-medium hover:bg-blue-700 transition">
                  Close
                </button>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
          <div className="max-w-2xl rounded-2xl bg-white shadow-2xl border border-gray-200 overflow-hidden">
            <div className="flex items-center justify-between px-6 py-5 border-b bg-gradient-to-r from-[#0072bc] to-blue-600 text-white">
              ...
              <button onClick={() => setEditForm(emptyForm)} className="w-9 h-9 rounded-full bg-white/20 hover:bg-white/30 transition flex items-center justify-center text-lg">
                ✕
              </button>
            </div>
            ...
            <form onSubmit={handleEditSubmit} className="p-6 space-y-5">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Category Name</label>
                <input readOnly value={editForm.categoryName} className="w-full rounded-xl border border-gray-300 bg-gray-100 px-4 py-3 text-gray-600 outline-none" />
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {[
                    ...
                    <label key={item.title} className="group cursor-pointer rounded-2xl border-2 border-dashed text-gray-800 border-gray-300 hover:border-blue-500 bg-gray-50 hover:bg-blue-50 transition p-5 text-center">
                      {item.current ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img
              ...
              <div className="flex justify-end gap-3 pt-5 border-t">
                <button type="button" onClick={() => setEditForm(emptyForm)} className="px-5 py-2.5 rounded-xl bg-gray-200 text-gray-700 hover:bg-gray-300 font-medium transition">
                  Cancel
                </button>
                <button type="submit" disabled={loading} className={`px-6 py-2.5 rounded-xl text-white font-medium transition ${loading ? "bg-blue-300 cursor-not-allowed" : "bg-[#0072bc] hover:bg-blue-700"}`}>
