...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full h-24 left-0 right-0" style={{ width: "100%", maxWidth: "100%", boxSizing: "border-box" }}>
      {/* ── Promo bar ── */}
      <div className="w-full bg-[#FFF8E7] border-b border-[#f0e6c8] h-9 overflow-hidden" style={{ width: "100%", boxSizing: "border-box" }}>
        ...
        <div className="w-full max-w-360 mx-auto h-14 px-4 sm:px-6 lg:px-8 flex items-center justify-between gap-4">
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3 lg:gap-6 shrink-0 ">
              ...
              <div className="relative hidden sm:block mr-5" ref={categoryRef}>
                <button onClick={() => setCategoryOpen(!categoryOpen)} className="h-10 px-2.5 lg:px-3 flex items-center gap-1.5 lg:gap-2 rounded-md hover:bg-[#F5F8F6] transition text-[#1a1a1a]">
                  <GridIcon className="w-4 h-4 text-[#266A3F] font-semibold " />
                  <span className="hidden lg:inline text-[14px] text-[#266A3F] font-semibold whitespace-nowrap">Browse All Categories</span>
                  <span className="hidden md:inline lg:hidden text-[12px] font-semibold whitespace-nowrap">Categories</span>
                  <ChevronDownIcon />
                </button>

                <div
                  className={`absolute top-full left-0 mt-1 w-50 bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden z-50
                ...
                >
                  {categories.map((item) => (
                    <Link key={item.categoryId || item.categoryName} href={`/products?category=${encodeURIComponent(item.categoryName)}`} onClick={() => setCategoryOpen(false)} className="block px-5 py-3 text-[13px] text-[#00462C] hover:bg-[#F5F8F6] transition ">
                      {item.categoryName}
                    </Link>
                  ))}
                </div>
            ...
            <div className="flex items-center gap-4 sm:gap-5 text-[#1a1a1a] shrink-0">
              ...
              {isLoggedIn ? (
                <>
                  ...
                  <Link href="/cart" className="relative hover:text-[#00462C] transition-colors" aria-label="Cart">
              ...
              <button className="lg:hidden hover:text-[#00462C] transition-colors" onClick={() => setMobileMenuOpen(true)} aria-label="Open menu">
                <MenuIcon />
              </button>
            </div>
          </div>
      ...
      {mobileMenuOpen && <div className="fixed inset-0 bg-black/40 z-40 lg:hidden" onClick={() => setMobileMenuOpen(false)} />}
          ...
          {NAV_LINKS.map((link) => (
            <div key={link.label} className="px-5 ">
              {link.hasDropdown ? (
                ...
                <a href={link.href} onClick={() => setMobileMenuOpen(false)} className="block py-3 text-[15px] font-medium text-[#1a1a1a] hover:text-[#462000] transition-colors">
                  {link.label}
                </a>
              )}
            </div>
