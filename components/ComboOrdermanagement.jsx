...
export default function ComboOrdermanagement() {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/admin/combo-orders?${query.toString()}`);
  ...
  const updateOrder = useCallback(
    async (orderId, patch) => {
      try {
        ...
        const response = await fetch(`/api/admin/combo-orders/${orderId}`, {
          method: "PATCH",
          headers: {
  ...
  const openOrderDetails = async (order) => {
    try {
      ...
      const res = await fetch(`/api/admin/combo-orders/${order.id}`);
