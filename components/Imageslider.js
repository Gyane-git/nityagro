    ...
    const fetchCardBanners = async () => {
      try {
        ...
        const mapped = picked
          ...
          .map((item) => ({
            ...item,
            src: /^https?:\/\//i.test(item.src) ? item.src : item.src.startsWith("/") ? item.src : `/${item.src}`,
          }));
        ...
        <div className="flex h-full gap-4">
          {fixedSlides.map((slide, i) => (
            <div key={`${slide.src}-${i}`} className="flex-1 relative overflow-hidden rounded-md lg:rounded-md aspect-[442/213]">
              <Image src={slide.src} alt={slide.alt || ""} fill className="object-contain" sizes="(max-width: 768px) 100vw, 33vw" priority={i < 3} unoptimized={slide.src?.startsWith("/banners/")} />
            </div>
          ))}
        </div>
      </div>
